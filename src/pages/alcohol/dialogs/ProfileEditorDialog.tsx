  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="mx-4 rounded-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Scale className="w-5 h-5 text-secondary" /> Parametres</DialogTitle>
        </DialogHeader>
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground text-center">Utilise ton poids actuel pour des estimations precises</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">Sexe (estimation BAC)</label>
            <div className="grid gap-2">
              {sexOptions.map(option => (
                <button key={option.value} onClick={() => setForm(p => ({ ...p, sex: option.value as 'male' | 'female' | 'unspecified' }))} className={cn("flex items-center justify-between p-3 rounded-xl border transition-all text-left", form.sex === option.value ? "bg-secondary/20 border-secondary text-secondary" : "bg-white/5 border-white/10 hover:bg-white/10")}>
                  <div>
          ...
          <Button onClick={async () => { await onUpdateProfile(form); onOpenChange(false); }} className="w-full rounded-xl h-12 bg-secondary hover:bg-secondary/80"><Check className="w-4 h-4 mr-2" /> Enregistrer</Button>
        </div>
      </DialogContent>
    </Dialog>
  );

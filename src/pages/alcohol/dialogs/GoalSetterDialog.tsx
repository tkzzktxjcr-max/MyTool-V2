      ...
      <DialogContent className="mx-4 rounded-2xl">
        ...
        <div className="space-y-4">
          <div className="grid gap-2">
            {presets.map(preset => (
              <button key={preset.value} onClick={() => setLimit(preset.value)} className={cn("flex items-center justify-between p-3 rounded-xl border transition-all text-left", limit === preset.value ? "bg-secondary/20 border-secondary text-secondary" : "bg-white/5 border-white/10 hover:bg-white/10")}>
                <div>
          ...
          <p className="text-xs text-muted-foreground text-center py-2">L'OMS recommande maximum 14 unites/semaine pour un equilibre sain</p>
          <Button onClick={async () => { await onSetGoal(limit); onOpenChange(false); }} className="w-full rounded-xl h-12 bg-secondary hover:bg-secondary/80"><Check className="w-4 h-4 mr-2" /> Enregistrer</Button>
        </div>
      </DialogContent>
    </Dialog>
  );

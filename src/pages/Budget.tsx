...
import { Plus, Wallet, ArrowUpRight, ArrowDownRight, PieChart as PieChartIcon, Trash2, Wine } from 'lucide-react';
...
export default function BudgetPage() {
  ...
  const { logs, loadData, getWeeklyUnits } = useAlcohol();
  ...
  // Calculate alcohol expenses from logs (estimated based on drink type)
  const calculateAlcoholExpenses = () => {
    // This is an estimation - in a real app, you'd track actual prices
    const avgPricePerUnit = 3; // Average price per alcohol unit in euros
    const weeklyUnits = getWeeklyUnits();
    return Math.round(weeklyUnits * avgPricePerUnit * 4 * 100) / 100; // Monthly estimate
  };

  const alcoholExpenses = calculateAlcoholExpenses();
  const totalWithAlcohol = totalExpenses + alcoholExpenses;
  const alcoholPercentage = totalWithAlcohol > 0 ? (alcoholExpenses / totalWithAlcohol) * 100 : 0;
  ...
  // Add alcohol to pie data if there's any expense
  if (alcoholExpenses > 0) {
    pieData.push({
      name: 'Alcool',
      value: alcoholExpenses,
      icon: '🍷'
    });
  }
    ...
    <div className="space-y-4 md:space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild><Button size="sm" className="w-full sm:w-auto"><Plus className="w-4 h-4 mr-1" />Nouvelle entree</Button></DialogTrigger>
          <DialogContent className="mx-4">
            <DialogHeader><DialogTitle>{type === 'expense' ? 'Nouvelle depense' : 'Nouveau revenu'}</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex gap-1 p-1 bg-muted rounded-xl">
                <button type="button" onClick={() => { setType('expense'); setFormData(p => ({ ...p, type: 'expense' })); }} className={cn("flex-1 py-2 rounded-lg text-sm font-medium", type === 'expense' ? "bg-destructive/20 text-destructive" : "text-muted-foreground")}>Depense</button>
          ...
          <CardContent className="p-4 md:p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-semibold text-sm md:text-base">Depenses alcool estimees</h3>
                  <p className="text-xs text-muted-foreground">Basées sur ta consommation</p>
                </div>
              </div>
              <span className="text-xl font-bold text-secondary">{alcoholExpenses.toFixed(2)} EUR</span>
            </div>
            <div className="h-2 rounded-full bg-white/10 overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(alcoholPercentage, 100)}%` }}
                transition={{ duration: 0.8 }}
                className="h-full rounded-full bg-secondary"
              />
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              {alcoholPercentage.toFixed(1)}% de tes depenses mensuelles
            </p>
          </CardContent>
        </Card>
      </motion.div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        <Card className="p-3 md:p-0">
          <CardContent className="pt-4 md:pt-6">
            <div className="flex items-center gap-2 text-xs md:text-sm text-muted-foreground mb-1"><ArrowDownRight className="h-3 w-3 md:h-4 md:w-4 text-destructive" />Depenses</div>
            <div className="text-lg md:text-2xl font-bold text-destructive">-{totalExpenses.toFixed(2)}EUR</div>
          </CardContent>
        </Card>
        <Card className="p-3 md:p-0">
          <CardContent className="pt-4 md:pt-6">
            ...
            <div className="text-lg md:text-2xl font-bold text-secondary">+{totalIncome.toFixed(2)}EUR</div>
          </CardContent>
        </Card>
        <Card className="p-3 md:p-0">
          <CardContent className="pt-4 md:pt-6">
            ...
            <div className={cn("text-lg md:text-2xl font-bold", balance >= 0 ? "text-secondary" : "text-destructive")}>{balance >= 0 ? '+' : ''}{balance.toFixed(2)}EUR</div>
          </CardContent>
        </Card>
        <Card className="p-3 md:p-0">
          <CardContent className="pt-4 md:pt-6">
            <div className="text-xs md:text-sm text-muted-foreground mb-1">Budget total utilise</div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <Card>
          <CardContent className="p-3 md:p-5">
            <h3 className="font-semibold flex items-center gap-2 mb-3 text-sm md:text-base"><PieChartIcon className="h-4 w-4 md:h-5 md:w-5" />Repartition</h3>
            {pieData.length === 0 ? (
              <div className="text-center py-6"><PieChartIcon className="w-8 h-8 md:w-10 md:h-10 mx-auto text-muted-foreground/30 mb-2" /><p className="text-muted-foreground text-sm">Aucune depense</p></div>
            ) : (
              <div className="space-y-2">
                {pieData.map((item, index) => (
                  ...
                  >
                    ...
                    <span className="font-medium">{item.value.toFixed(0)}EUR</span>
                  </motion.div>
                ))}
              </div>
            )}
        ...
        <Card>
          <CardContent className="p-3 md:p-5">
            <h3 className="font-semibold mb-3 text-sm md:text-base">Dernieres transactions</h3>
                ...
                {recentEntries.map(entry => (
                  <div key={entry.id} className="flex items-center gap-3 p-2 md:p-3 rounded-xl bg-white/[0.03]">
                    ...
                    <div className={cn("font-semibold text-xs md:text-sm", entry.type === 'expense' ? "text-destructive" : "text-secondary")}>
                      {entry.type === 'expense' ? '-' : '+'}{entry.amount.toFixed(2)}EUR
                    </div>

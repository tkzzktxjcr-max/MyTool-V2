...
export const useAlcohol = () => {
  ...
  const createDrink = useCallback(async (form: CreateDrinkForm, emoji?: string) => {
    ...
    const drink = await drinksService.createDrink({ name: form.name, type: form.type, abv: form.abv, defaultServingSize: form.defaultServingSize, emoji: emoji || '🥤', country: form.country, userId: user.$id });
    setDrinks(prev => [...prev, drink]);
    return drink;
  }, [user?.$id]);

  const quickLog = useCallback(async (drink: Drink, mood?: MoodType, quantity: number = 1, timestamp?: string) => {
    if (!user?.$id) throw new Error('Not authenticated');
    const log = await alcoholService.createLog(user.$id, { drinkType: drink.type, servingSize: drink.defaultServingSize, abv: drink.abv, mood, drinkName: drink.name, drinkEmoji: drink.emoji, quantity, timestamp });
  ...
  const undoDelete = useCallback(async () => {
    ...
    const restoredLog = await alcoholService.createLog(user.$id, { drinkType: lastDeletedLog.drinkType, servingSize: lastDeletedLog.servingSize, abv: lastDeletedLog.abv, mood: lastDeletedLog.mood, drinkName: lastDeletedLog.drinkName, drinkEmoji: lastDeletedLog.drinkEmoji, quantity: lastDeletedLog.quantity });
  ...
  const toggleFavorite = useCallback(async (drinkId: string) => {
    ...
    setDrinks(prev => prev.map(d => d.id === drinkId ? { ...d, isFavorite: !d.isFavorite, favoriteRank: d.isFavorite ? undefined : (d.favoriteRank || 1) } : d));
  }, []);

  const setWeeklyGoal = useCallback(async (limit: number) => {
    if (!user?.$id) throw new Error('Not authenticated');
    const updatedGoal = await goalsService.createOrUpdateGoal(user.$id, { weeklyLimit: limit, isActive: true });
    setGoal(updatedGoal);
  }, [user?.$id]);

  const updateUserProfile = useCallback(async (data: { weightKg?: number; sex?: 'male' | 'female' | 'unspecified' }) => {
    if (!user?.$id) throw new Error('Not authenticated');
    const updatedProfile = await profileService.createOrUpdateProfile(user.$id, data);
    setUserProfile(updatedProfile);
  }, [user?.$id]);

  const libraryDrinks = useMemo(() => drinks.filter(d => !d.userId).sort((a, b) => (b.popularity || 0) - (a.popularity || 0)), [drinks]);
  const userDrinks = useMemo(() => drinks.filter(d => d.userId === user?.$id).sort((a, b) => (b.usageCount || 0) - (a.usageCount || 0)), [drinks, user?.$id]);
  const favorites = useMemo(() => drinks.filter(d => d.isFavorite).sort((a, b) => (a.favoriteRank || 5) - (b.favoriteRank || 5)), [drinks]);
  ...
  const bacState = useMemo(() => {
    ...
    return { currentBAC: analysis.currentBAC, peakBAC: analysis.peakBAC, peakTime: analysis.peakTime, zeroTime: analysis.zeroTime, safeToDriveTime: analysis.safeToDriveTime, timeline: analysis.timeline, isAboveLimit: isAbove, isNearLimit: isNear };
  }, [logs, userProfile, refreshKey]);

  const insights = useMemo((): AlcoholInsight | null => alcoholService.calculateInsights(logs, goal), [logs, goal, refreshKey]);
  ...
  const isSafeToDrive = useMemo(() => (userProfile?.legalLimit || 0.5) >= bacState.currentBAC, [bacState.currentBAC, userProfile]);

  return { drinks, libraryDrinks, userDrinks, smartDrinks, favorites, recentlyUsed, suggestedFavorites, currentTimeOfDay, logs, goal, userProfile, loading, insights, lastDeletedLog, bacState, isSafeToDrive, loadData, createDrink, quickLog, deleteLog, undoDelete, deleteDrink, toggleFavorite, setWeeklyGoal, updateUserProfile, getTodayUnits, getWeeklyUnits };
};

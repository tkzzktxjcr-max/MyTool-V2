...
export const profileService = {
  async getProfile(userId: string): Promise<UserProfile | null> {
    ...
    return { id: doc.$id, userId: doc.userId, weightKg: doc.weightKg || 70, sex: (doc.sex as 'male' | 'female' | 'unspecified') || 'unspecified', legalLimit: doc.legalLimit || 0.5, onboardingCompleted: doc.onboardingCompleted || false, updatedAt: doc.$updatedAt };
  },
  ...
  async createOrUpdateProfile(userId: string, data: { weightKg?: number; sex?: 'male' | 'female' | 'unspecified'; legalLimit?: number; onboardingCompleted?: boolean }): Promise<UserProfile> {
    ...
    return { id: doc.$id, userId: doc.userId, weightKg: doc.weightKg || 70, sex: (doc.sex as 'male' | 'female' | 'unspecified') || 'unspecified', legalLimit: doc.legalLimit || 0.5, onboardingCompleted: doc.onboardingCompleted ?? false, updatedAt: doc.$createdAt };
  },
};